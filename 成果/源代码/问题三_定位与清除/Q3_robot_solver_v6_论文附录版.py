...
HTTP_RETRIES = 4
RETRY_SLEEP = 0.2
LOCAL_LOG = 'B3_strategy_log_stable.jsonl'
...
def clip_halfplane(poly: List[Point], a: float, b: float, c: float, eps: float=1e-09) -> List[Point]:
    """Sutherland-Hodgman 裁剪。保留满足 a*x + b*y + c >= 0 的半平面。"""
    if not poly:
        return []
...
def _coverage_veto_sample(scan_points: Sequence[Point]) -> float:
    """采样否决器。目标圆的多层极坐标网格上计算最大最近扫描点距离。提升消除率"""
    pts = list(scan_points)
    if not pts:
        return float('inf')
    worst = 0.0
...
def _shortest_open_path(start: Point, pts: Sequence[Point]) -> Tuple[float, List[int], float]:
    """Held-Karp 求从 start 出发访问全部 pts、无需返回的最短开放路径。同时返回最大单段长度。"""
    n = len(pts)
    if n == 0:
        return (0.0, [], 0.0)
    if n == 1:
...
class B3Solver:
    ...
    def coverage_scan_at(self, p: Point, with_reobserve: bool=False) -> None:
        """在 p 对所有尚未发现频道做一次完整扫描，并把 p 记入覆盖证据。同一点不会重复加入覆盖集合。"""
    ...
    def _best_anchor_plan(self, scan_points: Optional[Sequence[Point]]=None, remaining: Optional[Sequence[int]]=None, start: Optional[Point]=None) -> Tuple[float, float, List[int], float]:
        """
        从剩余固定环点中选择一个“足以完成覆盖认证”的最优子集。目标函数同时考虑：
          实际移动距离；
          每多一个完整扫描点，需要对所有当前未知频道再测一遍的时间；
          超长单段移动的惩罚。

        返回 (等效总代价m, 纯移动距离m, 环点访问顺序, 最大单段m)
        """
    ...
    def adaptive_search(self) -> None:
        ...
        self.status(f'认证布局 {self.layout_name} 搜索顺序: {tour}')
        tour_pos = 0
        ...
        if len(self.found_channels) < 16:
            ...
            if not coverage_complete(self.scan_points):
                raise RuntimeError(f'固定搜索结束但覆盖认证失败：exact={rad:.3f}m, sampled={sampled:.3f}m, witness={witness}')
            self.status(f'覆盖认证完成：exact≈{rad:.2f}m，sample≈{sampled:.2f}m')

    def choose_ring_tour(self) -> List[int]:
        """稳定固定覆盖路线，只优化“从哪边开始、最终停在哪个环点”。"""
...
def self_test() -> None:
    ...
    print(f'安全二次测点最坏目标距离 ≈ {worst2:.3f} m @ {worst_arg}')
    ...
    print(f'精确覆盖认证半径 = {cr:.6f} m，见证点={witness}')
    ...
    print(f'固定覆盖半径 = {fixed_cov:.6f} m，采样复核={fixed_sample:.6f} m')
    print(f'固定搜索路径 = {fixed_path:.6f} m')
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='2026 数模 B 题问题3自动搜索定位清除')
...
def main() -> None:
    ...
    if args.robot_id == '<参赛队号>':
        raise SystemExit('请先修改 ROBOT_ID，或以--robot-id传入队号')
